...
class BookingClass {
    ...
    public function update_template_sms() {
        $query =  $this->db->query("INSERT INTO `sms_templates`(`user_id`, `name`, `body1`, `SMSType`, `active`) VALUES ('$uid','$name','$body1','$smstype','$active')");
        return $query;
    }
    ...
    public function pay($item_id, $tid, $amount, $state, $track) {
        ...
        if ($this->check_txt_id($tid) == 0) {
           ...
           if(strpos($item_id, ",") == true){
            $items_id = explode(",", $item_id);
            $amounts = explode(",", $amount);
            $tracks_id = explode(",", $track);
            for($i=0; $i < count($items_id); $i++){
                $this->db->query("INSERT INTO `transactions`(transaction_id, transaction_amount, transaction_state, transaction_track, transaction_date,item_id) VALUES ('$tid','$amounts[$i]','$state','$tracks_id[$i]','$tdate','$items_id[$i]')");
            }
           }else{
    ...
    public function update_item($item_id) {
        $this->db->query("UPDATE add_courier SET payment_status = '1',status_courier='booked', act_status='2'  WHERE id IN ($item_id)");
    }
